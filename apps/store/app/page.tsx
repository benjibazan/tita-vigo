import Link from 'next/link';
import { CategoryCircles } from '@/components/products/CategoryCircles';
import { ProductGrid } from '@/components/products/ProductGrid';
import { getProducts, getCategories, Product, Category } from '@/lib/api';

export const revalidate = 60; // Revalidate every 60 seconds

export default async function HomePage() {
    let products: Product[] = [];
    let categories: Category[] = [];


    try {
        const [productsRes, categoriesRes] = await Promise.all([
            getProducts({ limit: 8 }),
            getCategories(),
        ]);
        products = productsRes.data || [];
        categories = categoriesRes.data || [];
    } catch (error) {
        console.error('Error fetching data:', error);
        // Use fallback data if API fails
        categories = [
            { id: '1', name: 'Anillos', slug: 'anillos' },
            { id: '2', name: 'Collares', slug: 'collares' },
            { id: '3', name: 'Aretes', slug: 'aretes' },
            { id: '4', name: 'Pulseras', slug: 'pulseras' },
        ];
    }

    // Map products to the format expected by ProductGrid
    const displayProducts = products.map((p: any) => ({
        id: p.id,
        name: p.name,
        slug: p.slug,
        price: Number(p.price),
        stock: p.stock,
        isPreOrder: p.isPreOrder,
        image: p.images?.[0]?.url,
    }));

    return (
        <div className="pb-20">
            {/* Hero Section */}
            <section className="relative bg-gradient-to-b from-pink-50 to-white py-12 px-4">
                <div className="max-w-6xl mx-auto text-center">
                    <h1 className="text-3xl md:text-5xl font-bold text-gray-900 mb-4">
                        Joyas que cuentan tu historia
                    </h1>
                    <p className="text-gray-600 text-lg mb-8 max-w-2xl mx-auto">
                        Descubre nuestra colección de joyas elegantes al alcance de todos
                    </p>
                    <Link
                        href="/productos"
                        className="inline-block bg-primary text-white px-8 py-3 rounded-full font-semibold hover:bg-primary-dark transition-colors"
                    >
                        Ver Colección
                    </Link>
                </div>
            </section>

            {/* Categories - Instagram Stories Style */}
            <section className="py-8 px-4">
                <div className="max-w-6xl mx-auto">
                    <CategoryCircles categories={categories} />
                </div>
            </section>

            {/* Featured Products */}
            <section className="py-8 px-4">
                <div className="max-w-6xl mx-auto">
                    <h2 className="text-2xl font-bold text-gray-900 mb-6">
                        Productos Destacados
                    </h2>
                    {displayProducts.length > 0 ? (
                        <ProductGrid products={displayProducts} />
                    ) : (
                        <p className="text-center text-gray-500 py-8">
                            Cargando productos...
                        </p>
                    )}
                </div>
            </section>

            {/* WhatsApp Float Button */}
            <a
                href="https://wa.me/51999999999?text=Hola!%20Me%20interesa%20conocer%20más%20sobre%20sus%20joyas"
                target="_blank"
                rel="noopener noreferrer"
                className="fixed bottom-20 right-6 md:bottom-6 bg-green-500 text-white p-4 rounded-full shadow-lg hover:bg-green-600 transition-colors z-40"
                aria-label="Contactar por WhatsApp"
            >
                <svg
                    className="w-6 h-6"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                >
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
            </a>
        </div>
    );
}
